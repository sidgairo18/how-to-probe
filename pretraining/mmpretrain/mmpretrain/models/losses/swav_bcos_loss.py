...
class SwAVBcosLoss(BaseModule):
    ...
    def __init__(self,
        ...
        if isinstance(num_prototypes, list):
            ...
            self.prototypes = BcosLinear(feat_dim, num_prototypes, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            w = self.prototypes.linear.weight.data.clone()
            w = nn.functional.normalize(w, dim=1, p=2)
            self.prototypes.linear.weight.copy_(w)
                    ...
                    if self.use_queue or not torch.all(self.queue[i,
                        ...
                        out = torch.cat(
                            (torch.mm(self.queue[i],
                                      self.prototypes.linear.weight.t()), out))
                    # fill the queue
