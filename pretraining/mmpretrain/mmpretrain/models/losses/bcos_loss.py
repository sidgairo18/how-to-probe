...
__all__ = [
    "BcosBinaryCrossEntropyLoss",
    "UniformOffLabelsBCEWithLogitsLoss",
]


@MODELS.register_module()
class BcosBinaryCrossEntropyLoss(nn.Module):
    ...
    def __init__(
        ...
        super(BcosBinaryCrossEntropyLoss, self).__init__()
...
class UniformOffLabelsBCEWithLogitsLoss(nn.Module):
    ...
    def forward(self, x: torch.Tensor, target: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        return F.binary_cross_entropy_with_logits(x, target.float(), reduction=self.reduction)
