...
# Note: for Bcos models have used LayerNorm instead of BatchNorm. Need to be sure if this is alright?
# The above was before, now have update with UncenteredBatchNorm1d :)
@MODELS.register_module()
class BcosNonLinearNeck(BaseModule):
    """The non-linear neck.

    Structure: bcosfc-bn-[bcosfc-bn] where the substructure in [] can be repeated.
    ...
    Args:
        ...
        norm_cfg (dict): Dictionary to construct and config norm layer.
            Defaults to dict(type='SyncBN'). # this is not for Bcos Models yet :(
    ...
    def __init__(
        ...
        super(BcosNonLinearNeck, self).__init__(init_cfg)
        self.with_avg_pool = with_avg_pool
        if with_avg_pool:
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            self.fc0 = BcosLinear(in_channels, hid_channels)
        else:
            # self.relu = nn.ReLU(inplace=True) # bcos models donot need relu :)
            self.fc0 = BcosLinear(in_channels, hid_channels)

        #self.bn0 = norms.NoBias(norms.DetachableLayerNorm)(hid_channels) # LayerNorm
        self.bn0 = norms.NoBias(norms.BatchNormUncentered1d)(hid_channels) # BatchNorm1d
    ...
    def forward(self, x: Tuple[torch.Tensor]) -> Tuple[torch.Tensor]:
        ...
        for fc_name, bn_name in zip(self.fc_names, self.bn_names):
            ...
            #x = self.relu(x) # no relu used in bcos models :)
            x = fc(x)
