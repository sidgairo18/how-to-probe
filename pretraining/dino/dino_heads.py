...
class BcosDINOHead(nn.Module):                                                                
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, nlayers=3, hidden_dim=2048, bottleneck_dim=256, linear_layer=BcosLinear, norm_layer=norms.NoBias(norms.DetachableLayerNorm), act_layer=nn.Identity):
        super().__init__()                                                                    
        nlayers = max(nlayers, 1)                                                             
        if nlayers == 1:                                                                      
            self.mlp = linear_layer(in_dim, bottleneck_dim)                                   
            #self.mlp = nn.Linear(in_dim, bottleneck_dim, bias=False)
        else:                                                                                 
