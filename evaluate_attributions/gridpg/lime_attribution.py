...
class Lime(lime_image.LimeImageExplainer):
    def __init__(
        self,
        model,
        num_samples=500,
        ...
        self.model = model
        if loss_type=='bce':
            self._model_to_probabilities = functools.partial(torch.sigmoid)
            print("loss type is bce using Sigmoid")
        else:
            self._model_to_probabilities = functools.partial(torch.softmax, dim=1)
            print("loss type is ce using Softmax")
        """
        self._model_to_probabilties = getattr(
            model, "to_probabilities", functools.partial(torch.softmax, dim=1)
        )
        """
        print("Using to_probabilities: ", self._model_to_probabilities)
        ...
        self.is_bcos_model = model_type == 'bcos'
    ...
    def pred_f(self, input_samples):
        ...
        out = self._model_to_probabilities(out_tensor)
    ...
    def attribute(self, img, target, return_all=False):
        ...
        if self.full_map:                                                                 
            if not return_all:                                                            
                full_map = np.zeros_like(explanation.segments, dtype=float)               
                masks = {                                                                 
                    s: explanation.segments == s                                          
                    for s in np.unique(explanation.segments)                              
                }                                                                         
                for idx, v in explanation.local_exp[int(target[0])]:                      
                    full_map[masks[idx]] = v                                              
                return torch.from_numpy(full_map)[None, None]                             
            else:                                                                         
                result = []                                                               
                for _tgt in target:                                                       
                    full_map = np.zeros_like(explanation.segments, dtype=float)           
                    ...
                    for idx, v in explanation.local_exp[_tgt]:                            
                        full_map[masks[idx]] = v                                          
                    result.append(full_map[None])                                         
                return torch.from_numpy(np.concatenate(result, axis=0))[:, None]          
    ...
    def attribute_selection(self, x, tgts):
        if self.full_map:
            # Only compute full map for tgts anyways
            return self.attribute(x, tgts, return_all=True)
