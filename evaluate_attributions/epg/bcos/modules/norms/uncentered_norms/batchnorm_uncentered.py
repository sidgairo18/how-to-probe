...
class BatchNormUncentered1d(BatchNormUncentered2d):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def forward(self, input: Tensor) -> Tensor:
        if input.dim() == 2:
            return super().forward(input.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1)
        elif input.dim() == 3:
            return super().forward(input.unsqueeze(-1)).squeeze(-1)
        else:
            raise ValueError(f"input must be 2 or 3 dim, got {input.dim()}")
