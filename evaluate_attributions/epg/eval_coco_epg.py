...
from eval_coco_multilabel_dataloader import *
from interpretability.utils import * # for bcos visualizations


# extra utils
object_categories = ['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train',
...
data_root='/path/to/COCO2014/'
data_split='val2014'
annotations_file='/path/to/coco/annotations/instances_val2014.json'
...
def main(
    ...
    dataset = image_loader(
            data_root+data_split,
            data_root+annotations_file,
    ...
    for loader_idx, (data, target) in enumerate(loader):
        ...
        target, idx_to_bbox, orig_dims, mask = target['labels'].squeeze(0), target['idx_to_bbox'], target['orig_dims'], target['mask']

        # parse mask to save memory
        mask_numpy_array = np.ones((data.shape[2], data.shape[3]))*255
        col_mask = np.zeros((data.shape[2], data.shape[3], 3))
        for lbl in mask:
            curr_mask = mask[lbl].squeeze(0).cpu().numpy()
            mask_numpy_array[curr_mask > 0] = lbl
            col_mask[curr_mask > 0] = dataset.metainfo['palette'][lbl]
            curr_mask = None
        mask = None

        orig_dims = [int(x) for x in orig_dims]
        for iii in idx_to_bbox:
            for jjj, _ in enumerate(idx_to_bbox[iii]):
                idx_to_bbox[iii][jjj] = [int(x) for x in idx_to_bbox[iii][jjj]]
        ...
        #cv2.imwrite(fname+"_segmask.png", col_mask.astype(np.uint8))
        ...
        for metric in ['epg', 'bbox_miou', 'seg_miou']:
